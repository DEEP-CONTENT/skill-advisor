...
`claudeskill-settings.json` if the advisor has written one, otherwise the value the session
actually launched with as reported by the sensor's first observation. When that value has
